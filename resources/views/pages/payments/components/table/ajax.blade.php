...
<tr id="payment_{{ $payment->payment_id }}">
    ...
    @endif
    <td class="payments_col_amount" id="payments_col_amount_{{ $payment->payment_id }}">{{ runtimeMoneyFormat($payment->payment_amount) }}</td>
    ...
    <td class="payments_col_action actions_column" id="payments_col_action_{{ $payment->payment_id }}">
        ...
        <span class="list-table-action dropdown font-size-inherit">
            ...
            <button type="button" title="{{ cleanLang(__('lang.delete')) }}" class="data-toggle-action-tooltip btn btn-outline-danger btn-circle btn-sm confirm-action-danger"
                data-confirm-title="{{ cleanLang(__('lang.delete_payment')) }}" data-confirm-text="{{ cleanLang(__('lang.are_you_sure')) }}"
                data-ajax-type="DELETE" data-url="{{ url('/') }}/payments/{{ $payment->payment_id }}">
                <i class="sl-icon-trash"></i>
            </button>
            @endif
            <a href="javascript:void(0)"
            title="{{ cleanLang(__('lang.view')) }}" class="data-toggle-action-tooltip btn btn-outline-info btn-circle btn-sm show-modal-button js-ajax-ux-request" data-toggle="modal"
                data-url="{{ url( '/') }}/payments/{{  $payment->payment_id }} " data-target="#plainModal"
