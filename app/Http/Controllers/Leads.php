...
class Leads extends Controller {
    ...
    public function convertLead(LeadConvert $request, ClientRepository $clientrepo, UserRepository $userrepo, $id) {
        ...
        if (!$client = $clientrepo->create(['return' => 'client'])) {
            abort(409);
        }
        ...
        $get = \App\Models\Item::Where('item_id', $id)->first();

        //create new user (client role = 3)
        request()->merge([
            'account_owner' => 'yes',
            'role_id' => 3,
            'type' => 'client',
        ...
        if (!$userid = $userrepo->create(bcrypt($password))) {
            abort(409);
        }
