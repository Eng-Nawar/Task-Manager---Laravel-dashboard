...
class Index {
    ...
    public function handle($request, Closure $next) {
        ...
        if (auth()->user()->is_team) {
            if ($project->assignedperm_milestone_manage == 'yes' || $project->check('super-user', request('milestoneresource_id'))) {
        ...
        //user permissions
        if ($this->projectpermissons->check('view', request('milestoneresource_id'))) {
            return $next($request);
        }
